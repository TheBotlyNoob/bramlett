		...
		switch (game.status.status) {
			case GraphQlGameStatusInner.Downloading:
				...
				return (
					<Tooltip
						showArrow
						content={`Downloading... (${game.status.progress![0] / 1000}GB out of ${
							game.status.progress![1] / 1000
						}GB)`}
					>
			...
			case GraphQlGameStatusInner.Installing:
				...
				return (
					<Tooltip
						showArrow
						content={`Installing... (${game.status.progress![0]} files out of ${
							game.status.progress![1]
						})`}
					>
			...
			case GraphQlGameStatusInner.NotDownloaded:
				return (
					<Button color="primary" onClick={() => downloadGame({ variables: { game: game.id } })}>
						Download
					</Button>
				);
			...
			case GraphQlGameStatusInner.Stopped:
				return (
					<ButtonGroup>
						<Button color="danger" onClick={() => deleteGame({ variables: { game: game.id } })}>
							Delete
						</Button>
						<Divider orientation="vertical" />
						<Button color="success" onClick={() => runGame({ variables: { game: game.id } })}>
							Start
						</Button>
					</ButtonGroup>
				);
			...
			{data && !error && (
				<>
					{process.env.NODE_ENV === 'development' && (
						<Button isIconOnly aria-label="Refresh game list" onClick={() => updateGameList()}>
						...
						{Object.values(data.games).map((game, key) => (
							<Card key={key} className="max-w-xs">
								<CardHeader className="whitespace-nowrap overflow-scroll">
									<span className="text-xl font-bold m-auto">{game.name}</span>
								</CardHeader>
								<CardBody>
									<Image src={game.icon} alt={`${game.name}'s icon`} className="m-0" />
									<Divider className="mt-3" />
								</CardBody>
								<CardFooter className="flex justify-center">{gameStatus(game)}</CardFooter>
							</Card>
						))}
					</div>
				</>
